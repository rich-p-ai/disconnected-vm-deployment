...
extract_disk_from_archive() {
  ...
  found="$(find "${tmp}" -type f \( -name 'disk.img' -o -name 'disk.img.gz' -o -name '*.raw' -o -name '*.qcow2' -o -name 'disk' \) | head -n1 || true)"
  if [[ -z "${found}" ]]; then
...
vmexport_download() {
  ...
  rm -f "${output}"
  if virtctl vmexport download "${EXPORT_NAME}" --namespace="${NS}" --volume="${export_vol}" --output="${output}" --format="${fmt}" --keep-vme --insecure --readiness-timeout=30m; then
    return 0
  fi
...
main() {
  ...
  if oc get vmi "${VM}" -n "${NS}" >/dev/null 2>&1; then
    ...
    virtctl stop "${VM}" -n "${NS}" || { echo "ERROR: stop failed. Run: virtctl stop ${VM} -n ${NS}" >&2; exit 1; }
    while oc get vmi "${VM}" -n "${NS}" >/dev/null 2>&1; do sleep 5; done
  else
    echo "VM ${NS}/${VM} is already stopped."
  fi
  ...
  [[ -s "${BUNDLE}/source-disks.tsv" ]] || { echo "ERROR: No PVC-backed disks." >&2; exit 1; }
  ...
  while IFS=$'\t' read -r VOLUME_NAME PVC_NAME; do
    ...
    case "${LOWER}" in
      *root*|*boot*|*os*|*system*|*c-drive*|*cdrive*) BOOT_VOLUME="${VOLUME_NAME}"; break ;;
    esac
...
EOF
  ...
  virtctl vmexport create "${EXPORT_NAME}" --vm="${VM}" --namespace="${NS}" --ttl=24h
  if ! oc wait virtualmachineexport "${EXPORT_NAME}" -n "${NS}" --for=jsonpath='{.status.phase}'=Ready --timeout=30m; then
  ...
  [[ -s "${EXPORT_INFO}" ]] || { echo "ERROR: Export published no volume links." >&2; exit 1; }
